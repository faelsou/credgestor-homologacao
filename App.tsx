...
// VERSÃO MINIMAL - SEM WEBHOOK - APENAS FUNCIONALIDADE BÁSICA
...
export const AppContext = React.createContext<{
  ...
  view: string;
  setView: (v: string) => void;
  theme: 'light' | 'dark-emerald' | 'dark-graphite';
  setTheme: (theme: 'light' | 'dark-emerald' | 'dark-graphite') => void;
}>({} as any);

const App: React.FC = () => {
  const [user, setUser] = useState<User | null>(null);
  const [view, setView] = useState('home');
  const [theme, setTheme] = useState<'light' | 'dark-emerald' | 'dark-graphite'>('light');
  const [loanToEditId, setLoanToEditId] = useState<string | null>(null);
  ...
  // ⭐ FUNÇÃO MINIMAL - SEM WEBHOOK, SEM COMPLICAÇÃO
  const addClient = useCallback(async (client: Client): Promise<Client | null> => {
    console.log('📝 addClient chamado:', client);
    
    // Simplesmente adiciona ao estado
    setClients(prev => {
      const updated = [...prev, client];
      console.log('✅ Cliente adicionado ao estado:', updated);
      return updated;
    });
    
    console.log('✅ addClient completado');
    return client;
  }, []);

  const updateClient = useCallback((client: Client) => {
    console.log('📝 updateClient chamado:', client);
  ...
  const addLoan = useCallback((loan: Loan, generatedInstallments: Installment[]) => {
    setLoans(prev => [...prev, loan]);
    setInstallments(prev => [...prev, ...generatedInstallments]);
  }, []);

  const updateLoan = useCallback((loan: Loan, generatedInstallments: Installment[]) => {
    setLoans(prev => prev.map(l => l.id === loan.id ? loan : l));
    setInstallments(prev => prev.filter(i => i.loanId !== loan.id).concat(generatedInstallments));
  }, []);
  ...
  const payInstallment = useCallback((id: string, amount?: number) => {
    setInstallments(prev => prev.map(inst => {
      if (inst.id !== id) return inst;
      const paidAmount = Math.min(inst.amount, (inst.amountPaid || 0) + (amount ?? inst.amount));
      const isPaid = paidAmount >= inst.amount;
      return {
        ...inst,
        status: isPaid ? InstallmentStatus.PAID : InstallmentStatus.PARTIAL,
  ...
  const scheduleFuturePayment = useCallback((id: string, reason: string, amount: number, date?: string) => {
    setInstallments(prev => prev.map(inst => {
      if (inst.id !== id) return inst;
      return {
        ...inst,
        ...
        promisedPaymentDate: date || getTodayDateString()
      };
    }));
  }, []);

  const startEditingLoan = useCallback((loanId: string) => {
    setLoanToEditId(loanId);
    setView('loans');
  }, []);
  ...
  const value = useMemo(() => ({
    ...
    theme,
    setTheme
  }), [user, clients, loans, installments, view, theme, login, logout, addClient, updateClient, deleteClient, addLoan, updateLoan, deleteLoan, payInstallment, scheduleFuturePayment, startEditingLoan, addUser, removeUser]);
  ...
  return (
    <AppContext.Provider value={value}>
      <DashboardLayout>
        {view === 'home' && <DashboardHome />}
        {view === 'clients' && <ClientsView />}
        {view === 'loans' && <LoansView editingLoanId={loanToEditId} onCloseEdit={() => setLoanToEditId(null)} />}
        {view === 'installments' && <InstallmentsView />}
        {view === 'users' && <UsersView />}
        {view === 'loanHistory' && <LoanHistoryView />}
      </DashboardLayout>
