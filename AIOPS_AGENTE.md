...
O monitor varre os serviços com a label `com.docker.stack.namespace=credgestor`
a cada ~30 segundos e classifica:
...
- Um incidente em aberto alerta na entrada, quando a severidade **aumenta** e
...
| `unavailable`, `degraded` | Forçar reinício das tasks (`force_update`) |
...
`task_failed` gera apenas alerta, sem ação automática (a falha já foi tratada
pelo Swarm ao recriar a task).
...
├── monitor.py              Detecção de falhas e indisponibilidade via Docker socket
