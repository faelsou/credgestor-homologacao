...
def listar_tenants(ativo: Optional[bool] = None, db: DatabaseConnection = Depends(get_db)):
...
def criar_tenant(tenant: TenantCreate, db: DatabaseConnection = Depends(get_db)):
    ...
    return {"success": True, "data": novo_tenant, "message": "Tenant criado com sucesso"}

@app.put("/api/tenants/{tenant_id}", tags=["Tenants"])
def atualizar_tenant(tenant_id: int, tenant: TenantUpdate, db: DatabaseConnection = Depends(get_db)):
    """Atualiza dados do tenant"""
    cruds = obter_cruds(db)
    tenant_atualizado = cruds['tenant'].update(tenant_id, **tenant.dict(exclude_unset=True))
    if not tenant_atualizado:
        raise HTTPException(status_code=404, detail="Tenant não encontrado")
    return {"success": True, "data": tenant_atualizado, "message": "Tenant atualizado com sucesso"}
...
def criar_usuario(
    ...
    return {"success": True, "data": novo_usuario, "message": "Usuário criado com sucesso"}
...
def atualizar_usuario(
    ...
    if not usuario_atualizado:
        raise HTTPException(status_code=404, detail="Usuário não encontrado")
    return {"success": True, "data": usuario_atualizado, "message": "Usuário atualizado com sucesso"}
...
def listar_clientes(
    ...
    cruds = obter_cruds(db)
    clientes = cruds['cliente'].list_by_tenant(tenant_id, ativo=ativo, tipo_pessoa=tipo_pessoa)
...
def criar_cliente(
    ...
    return {"success": True, "data": novo_cliente, "message": "Cliente criado com sucesso"}
...
def atualizar_cliente(
    ...
    if not cliente_atualizado:
        raise HTTPException(status_code=404, detail="Cliente não encontrado")
    return {"success": True, "data": cliente_atualizado, "message": "Cliente atualizado com sucesso"}
...
def criar_proposta(
    ...
    cruds = obter_cruds(db)
    nova_proposta = cruds['proposta'].create(
        tenant_id=tenant_id,
        **proposta.dict()
    )
    if not nova_proposta:
        raise HTTPException(status_code=400, detail="Erro ao criar proposta")
    return {"success": True, "data": nova_proposta, "message": "Proposta criada com sucesso"}
...
def atualizar_proposta(
    ...
    if not proposta_atualizada:
        raise HTTPException(status_code=404, detail="Proposta não encontrada")
    return {"success": True, "data": proposta_atualizada, "message": "Proposta atualizada com sucesso"}
...
def rejeitar_proposta(
    ...
    return {"success": True, "data": proposta_rejeitada, "message": "Proposta rejeitada"}
...
def pagar_parcela(
    ...
    cruds = obter_cruds(db)
    parcela_atualizada = cruds['parcela'].registrar_pagamento(parcela_id, tenant_id, valor_pago)
    
    if not parcela_atualizada:
        raise HTTPException(status_code=404, detail="Parcela não encontrada")
    
    return {"success": True, "data": parcela_atualizada, "message": "Pagamento registrado com sucesso"}
