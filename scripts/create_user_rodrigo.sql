...
DECLARE
    ...
    v_name text := 'Rodrigo Conecte Loja';
BEGIN
    ...
    v_slug := substring(v_slug || '-' || substring(v_tenant_id::text, 1, 8) from 1 for 50);
    
    -- Insere o novo tenant
    ...
    RAISE NOTICE '✅ Tenant criado exclusivamente para o usuário';
    RAISE NOTICE '   Tenant ID: %', v_tenant_id;
    RAISE NOTICE '   Nome: Aplicação - %', v_name;
...
DECLARE
    ...
    v_tenant_id uuid;
    v_email text := 'rodrigoconecteloja@gmail.com';
    v_password text := 'RodConect2026';
    v_name text := 'Rodrigo Conecte Loja';
    v_telefone text := '11992858462';
    v_role text := 'admin';
BEGIN
    -- Obter tenant_id criado anteriormente
    ...
    -- Se não encontrou, tenta criar novamente
    IF v_tenant_id IS NULL THEN
        ...
        VALUES (
            v_tenant_id,
            'Aplicação - ' || v_name,
            lower(regexp_replace(split_part(v_email, '@', 1), '[^a-z0-9]', '-', 'g')) || '-' || substring(v_tenant_id::text, 1, 8),
            v_email,
            '',
            true
        )
    ...
    SET 
        ...
        name = EXCLUDED.name,
        role = EXCLUDED.role,
        metadata = EXCLUDED.metadata || jsonb_build_object('telefone', v_telefone),
        updated_at = now();
    ...
    RAISE NOTICE 'Telefone: %', v_telefone;
...
--     AND tu.email = 'rodrigoconecteloja@gmail.com'
...
--     AND u.email = 'rodrigoconecteloja@gmail.com'
--     AND u.id != au.id;
