#!/usr/bin/env python3
"""
Script para criar usuário com tenant próprio
REGRA IMPORTANTE: Cada usuário deve ter sua própria aplicação separadamente.

Este script garante que cada novo usuário receba seu próprio tenant único.
"""
...
def create_user_with_tenant(user_data: dict):
    """
    Função principal que cria um usuário com seu próprio tenant.
    
    Args:
        user_data: Dicionário com:
            - email: Email do usuário
            - password: Senha do usuário
            - name: Nome do usuário
            - role: Role do usuário (padrão: 'admin')
    
    Returns:
        dict com user_id e tenant_id se sucesso, None caso contrário
    """
    email = user_data.get("email")
    password = user_data.get("password")
    name = user_data.get("name", email.split("@")[0] if email else "Usuário")
    role = user_data.get("role", "admin")
    
    if not email or not password:
        print("❌ Erro: email e password são obrigatórios")
        return None
    
    print("=" * 70)
    print("🔧 CRIANDO USUÁRIO COM TENANT PRÓPRIO")
    print("=" * 70)
    print(f"📋 REGRA IMPORTANTE: Cada usuário recebe sua própria aplicação")
    print(f"Email: {email}")
    print(f"Nome: {name}")
    print(f"Role: {role}")
    print()
    ...
    tenant_id = create_unique_tenant(supabase, email, name)
    if not tenant_id:
    ...
    auth_user = create_auth_user(email, password, name, role, tenant_id)
    if not auth_user:
    ...
    create_user_in_public_users(supabase, user_id, email, name, role, tenant_id)
    print()
    
    # 4. Criar vínculo em public.tenant_users
    print("📋 Passo 4: Criando vínculo em public.tenant_users...")
    create_tenant_user_link(supabase, tenant_id, user_id, email, name, role)
    print()
    
    # 5. Verificação final
    print("=" * 70)
    print("✅ USUÁRIO CRIADO COM TENANT PRÓPRIO!")
    print("=" * 70)
    print(f"Email: {email}")
    print(f"User ID: {user_id}")
    print(f"Tenant ID: {tenant_id}")
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Uso: python create_user_with_tenant.py <email> <password> [name] [role]")
        print()
        print("Exemplo:")
        print("  python create_user_with_tenant.py usuario@exemplo.com Senha123! 'Nome Usuário' admin")
        sys.exit(1)
