...
class TenantCRUD:
    ...
    def create(self, nome: str, slug: str, cnpj: Optional[str] = None, 
               email: Optional[str] = None, telefone: Optional[str] = None,
               endereco: Optional[str] = None) -> Optional[Dict]:
        """Cria um novo tenant"""
    ...
    def list_all(self, ativo: Optional[bool] = None) -> List[Dict]:
        ...
        try:
            if ativo is not None:
                cursor.execute("SELECT * FROM tenants WHERE ativo = %s ORDER BY nome;", (ativo,))
            else:
    ...
    def update(self, tenant_id: int, **kwargs) -> Optional[Dict]:
        ...
        try:
            ...
            allowed_fields = ['nome', 'cnpj', 'email', 'telefone', 'endereco', 'ativo', 'configuracoes']
...
class UsuarioCRUD:
    ...
    def create(self, tenant_id: int, nome: str, email: str, senha_hash: str,
               cpf: Optional[str] = None, telefone: Optional[str] = None,
               cargo: Optional[str] = None, tipo_usuario: str = 'user') -> Optional[Dict]:
        ...
        try:
            cursor.execute("""
                ...
                RETURNING *;
            """, (tenant_id, nome, email, senha_hash, cpf, telefone, cargo, tipo_usuario))
            self.db.conn.commit()
            return dict(cursor.fetchone())
        except Exception as e:
            self.db.conn.rollback()
    ...
    def list_by_tenant(self, tenant_id: int, ativo: Optional[bool] = None) -> List[Dict]:
    ...
    def update(self, usuario_id: int, tenant_id: int, **kwargs) -> Optional[Dict]:
        ...
        cursor = self.db.get_cursor()
        try:
            allowed_fields = ['nome', 'email', 'cpf', 'telefone', 'cargo', 'ativo', 'tipo_usuario']
            updates = {k: v for k, v in kwargs.items() if k in allowed_fields}
...
class ClienteCRUD:
    ...
    def create(self, tenant_id: int, nome: str, cpf_cnpj: str, tipo_pessoa: str,
        ...
        try:
            ...
            # Campos opcionais
            optional_fields = ['email', 'telefone', 'celular', 'endereco', 'cidade', 
                             'estado', 'cep', 'data_nascimento', 'renda_mensal', 
                             'profissao', 'observacoes', 'score_credito']
    ...
    def list_by_tenant(self, tenant_id: int, ativo: Optional[bool] = None,
                       tipo_pessoa: Optional[str] = None) -> List[Dict]:
    ...
    def update(self, cliente_id: int, tenant_id: int, **kwargs) -> Optional[Dict]:
        ...
        cursor = self.db.get_cursor()
        try:
            allowed_fields = ['nome', 'email', 'telefone', 'celular', 'endereco', 
                            'cidade', 'estado', 'cep', 'data_nascimento', 'renda_mensal',
                            'profissao', 'observacoes', 'score_credito', 'ativo']
...
class PropostaCRUD:
    ...
    def create(self, tenant_id: int, cliente_id: int, produto_id: int, 
               usuario_id: int, numero_proposta: str, valor_solicitado: float,
               **kwargs) -> Optional[Dict]:
        """Cria uma nova proposta"""
        cursor = self.db.get_cursor()
        try:
            fields = ['tenant_id', 'cliente_id', 'produto_id', 'usuario_id', 
                     'numero_proposta', 'valor_solicitado']
            values = [tenant_id, cliente_id, produto_id, usuario_id, 
                     numero_proposta, valor_solicitado]
            
            optional_fields = ['valor_aprovado', 'taxa_juros', 'prazo', 'valor_parcela',
                             'data_primeira_parcela', 'status', 'observacoes']
    ...
    def list_by_tenant(self, tenant_id: int, status: Optional[str] = None) -> List[Dict]:
    ...
    def update(self, proposta_id: int, tenant_id: int, **kwargs) -> Optional[Dict]:
        ...
        cursor = self.db.get_cursor()
        try:
            allowed_fields = ['valor_aprovado', 'taxa_juros', 'prazo', 'valor_parcela',
                            'data_primeira_parcela', 'status', 'observacoes']
    ...
    def aprovar(self, proposta_id: int, tenant_id: int, valor_aprovado: float,
                taxa_juros: float, prazo: int, valor_parcela: float) -> Optional[Dict]:
        """Aprova uma proposta"""
        return self.update(proposta_id, tenant_id, 
                          status='aprovado',
    ...
    def rejeitar(self, proposta_id: int, tenant_id: int, observacoes: str) -> Optional[Dict]:
...
class ParcelaCRUD:
    ...
    def create_parcelas_proposta(self, proposta_id: int, tenant_id: int) -> bool:
        ...
        try:
            ...
            for num in range(1, prazo + 1):
                ...
                """, (tenant_id, proposta_id, num, valor_parcela, data_primeira, num - 1))
    ...
    def registrar_pagamento(self, parcela_id: int, tenant_id: int, 
                           valor_pago: float) -> Optional[Dict]:
